//! Views tab — select views and show dependencies.

use crate::components::{checkbox_list, output_actions, output_panel, search_bar, section_header, selection_toolbar};
use crate::db;
use crate::style;
use eframe::egui;
use sqlx::PgPool;
use std::collections::HashSet;

/// State for the Views tab.
pub struct ViewsState {
    pub views: Vec<(String, String)>, // (name, kind)
    pub search: String,
    pub selected: HashSet<String>,
    pub output: String,
    pub loading: bool,
}

impl Default for ViewsState {
    fn default() -> Self {
        Self {
            views: vec![],
            search: String::new(),
            selected: HashSet::new(),
            output: String::new(),
            loading: false,
        }
    }
}

impl ViewsState {
    pub fn clear(&mut self) {
        self.views.clear();
        self.search.clear();
        self.selected.clear();
        self.output.clear();
    }

    pub fn load_views(&mut self, rt: &tokio::runtime::Runtime, pool: &PgPool, schema: &str) {
        self.views = rt
            .block_on(db::list_views(pool, schema))
            .unwrap_or_default();
        self.selected.clear();
        self.output.clear();
        self.search.clear();
    }

    pub fn load_deps(
        &mut self,
        rt: &tokio::runtime::Runtime,
        pool: &PgPool,
        schema: &str,
    ) {
        let mut selected: Vec<String> = self.selected.iter().cloned().collect();
        selected.sort();
        self.loading = true;
        let mut output = String::new();

        for view in &selected {
            match rt.block_on(db::view_dependencies(pool, schema, view)) {
                Ok((deps, definition)) => {
                    if !output.is_empty() {
                        output.push('\n');
                    }
                    output.push_str(&format!(
                        "═══ Dependencies for '{schema}.{view}' ═══\n\n"
                    ));

                    if deps.is_empty() {
                        output.push_str(
                            "  No dependencies (view may reference only literals or functions).\n",
                        );
                    } else {
                        output.push_str(&format!("  {:<5} {:<22} {}\n", "#", "Kind", "Object"));
                        output.push_str(&format!("  {}\n", "─".repeat(55)));
                        for (i, dep) in deps.iter().enumerate() {
                            let qualified = if dep.schema == schema {
                                dep.name.clone()
                            } else {
                                format!("{}.{}", dep.schema, dep.name)
                            };
                            output.push_str(&format!(
                                "  {:<5} {:<22} {}\n",
                                i + 1,
                                dep.kind,
                                qualified
                            ));
                        }
                    }

                    if let Some(def) = definition {
                        output.push_str(&format!(
                            "\n  -- View definition --\n  {}\n",
                            def.replace('\n', "\n  ")
                        ));
                    }
                }
                Err(e) => {
                    output.push_str(&format!("-- Error for {view}: {e}\n"));
                }
            }
        }

        self.output = output;
        self.loading = false;
    }

    pub fn draw(
        &mut self,
        ui: &mut egui::Ui,
        rt: &tokio::runtime::Runtime,
        pool: &PgPool,
        schema: &str,
    ) {
        let available = ui.available_size();
        let left_width = (available.x * style::PANEL_LEFT_RATIO).max(200.0);

        ui.horizontal(|ui| {
            // Left: view selection
            ui.vertical(|ui| {
                ui.set_width(left_width);
                ui.set_min_height(available.y);

                section_header(ui, "Select Views", self.views.len(), "total");
                ui.add_space(2.0);
                search_bar(ui, &mut self.search, "Filter views...");
                ui.add_space(2.0);

                let (select_all, clear) = selection_toolbar(ui, self.selected.len());
                if select_all {
                    let s = self.search.to_lowercase();
                    for (name, _) in &self.views {
                        if s.is_empty() || name.to_lowercase().contains(&s) {
                            self.selected.insert(name.clone());
                        }
                    }
                }
                if clear {
                    self.selected.clear();
                }
                ui.add_space(2.0);

                let items: Vec<(String, String)> = self
                    .views
                    .iter()
                    .map(|(n, k)| (n.clone(), k.clone()))
                    .collect();
                let toggles = checkbox_list(ui, "view_select", &items, &self.selected, &self.search);
                for (name, checked) in toggles {
                    if checked {
                        self.selected.insert(name);
                    } else {
                        self.selected.remove(&name);
                    }
                }
            });

            ui.separator();

            // Right: dependency output
            ui.vertical(|ui| {
                section_header(ui, "Dependencies", 0, "");
                ui.add_space(2.0);

                let can = !self.selected.is_empty();
                let output_ref = self.output.clone();
                let acted = output_actions(
                    ui,
                    "🔍 Show Dependencies",
                    can,
                    self.loading,
                    &output_ref,
                    Some("view_dependencies.txt"),
                );
                if acted {
                    self.load_deps(rt, pool, schema);
                }

                ui.add_space(4.0);
                let output = self.output.clone();
                output_panel(
                    ui,
                    "deps_output",
                    &output,
                    "Select views and click 🔍 Show Dependencies.",
                );
            });
        });
    }
}
